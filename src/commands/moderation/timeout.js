...
async function timeout(issuer, target, ms, reason) {
  ...
  if (typeof response === "boolean") return `${target.user.username} отправлен на тайм-аут!`;
  if (response === "BOT_PERM") return `У меня нет прав для тайм-аута ${target.user.username}`;
  else if (response === "MEMBER_PERM") return `У тебя нет прав для тайм-аута ${target.user.username}`;
  else if (response === "ALREADY_TIMEOUT") return `${target.user.username} уже отправлен на тайм-аут!`;
  else return `Ошибка тайм-аута ${target.user.username}`;
}
