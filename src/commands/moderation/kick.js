...
async function kick(issuer, target, reason) {
  ...
  if (typeof response === "boolean") return `${target.user.tag} кикнут!`;
  if (response === "BOT_PERM") return `У меня нет прав для кика ${target.user.tag}`;
  else if (response === "MEMBER_PERM") return `У тебя нет прав для кика ${target.user.tag}`;
  else return `Ошибка кика ${target.user.tag}`;
}
