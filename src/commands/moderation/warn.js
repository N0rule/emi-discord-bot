...
async function warn(issuer, target, reason) {
  ...
  if (typeof response === "boolean") return `${target.user.tag} Предупрежден!`;
  if (response === "BOT_PERM") return `У меня нет прав для предупреждения ${target.user.tag}`;
  else if (response === "MEMBER_PERM") return `У тебя нет прав для предупреждения ${target.user.tag}`;
  else return `Ошибка предупреждения ${target.user.tag}`;
}
