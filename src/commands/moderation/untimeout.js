...
async function untimeout(issuer, target, reason) {
  ...
  if (typeof response === "boolean") return `тайм-аут ${target.user.tag} удален!`;
  if (response === "BOT_PERM") return `У меня нет прав для удаления тайм-аута с ${target.user.tag}`;
  else if (response === "MEMBER_PERM") return `У тебя нет прав для удаления тайм-аута с ${target.user.tag}`;
  else if (response === "NO_TIMEOUT") return `${target.user.tag} не имеет тайм-аута!`;
  else return `Ошибка удаления тайм-аута с ${target.user.tag}`;
}
